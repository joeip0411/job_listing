...
resource "aws_glue_catalog_table" "brz__job_listing" {
  ...
  storage_descriptor {
    location                  = format("s3://%s/%s.db/brz__job_listing", aws_s3_bucket.datalake_storage_bucket.id, aws_glue_catalog_database.job.name)
...
import {
  to = aws_glue_catalog_table.brz__job_description
  id = "654654412098:job:brz__job_description"
}

resource "aws_glue_catalog_table" "brz__job_description" {
  catalog_id    = var.aws_account_id
  database_name = aws_glue_catalog_database.job.name
  name          = "brz__job_description"
  table_type         = "EXTERNAL_TABLE"
  open_table_format_input {
    iceberg_input {
      metadata_operation = "CREATE"
  ...
  storage_descriptor {
    ...
    location                  = format("s3://%s/%s.db/brz__job_description", aws_s3_bucket.datalake_storage_bucket.id, aws_glue_catalog_database.job.name)
